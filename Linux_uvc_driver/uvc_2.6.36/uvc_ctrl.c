...
#define RERVISION_9420_SERIES_CHIPID 0x90
#define RERVISION_9422_SERIES_CHIPID 0x92
...
int uvc_xu_ctrl_init(struct uvc_device *dev)
{
	...
	ret = uvc_xu_ctrl_add( &rervision_xu_sys_ctrls[i], &rervision_xu_sys_mappings[i]);
	if (ret < 0) return ret;
	...
	else if((chip == CHIP_9421)||(chip == CHIP_9422))
	{
...
int uvc_xu_ctrll_ReadChip(struct uvc_device *dev, int *chip)
{
	...
	*chip = CHIP_NONE;
	...
	xctrl.unit = 3;
	xctrl.selector = XU_RERVISION_SYS_ASIC_READ;
	xctrl.size = 4;
	xctrl.data = ctrldata;
	...
	if(xctrl.data[2] == RERVISION_9420_SERIES_CHIPID)
	{
		*chip = CHIP_9420;
	}	
	if(xctrl.data[2] == RERVISION_9422_SERIES_CHIPID)
	{
		...
		if(xctrl.data[2] == RERVISION_9422_DDR_64M)
			*chip = CHIP_9422;
		else if(xctrl.data[2] == RERVISION_9422_DDR_16M)
			*chip = CHIP_9421;			
	}
