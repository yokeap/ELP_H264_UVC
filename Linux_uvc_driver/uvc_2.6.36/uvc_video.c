...
#define PATCH_OF_9420_MJPG_EOF_LOST
	...
	struct uvc_buffer *buf)
{
	...
	for (i = 0; i < urb->number_of_packets; ++i) {
		...
		{
			if(((stream->dev->RER_Chip == CHIP_9421)||(stream->dev->RER_Chip == CHIP_9422))&& 
...
#ifdef PATCH_OF_9420_MJPG_EOF_LOST
			if(stream->dev->RER_Chip == CHIP_9420 && 
...
#define APPENDNUMBER 3 	// 2
			...
			else if(((stream->dev->RER_Chip == CHIP_9421)||(stream->dev->RER_Chip == CHIP_9422))&& 
				stream->cur_format->fcc == V4L2_PIX_FMT_MJPEG)
			{
				u8 Y_Remove_Size = 0, *UV_Quant_Start, *Next_Marker;
...
#endif // #ifdef PATCH_OF_9420_MJPG_EOF_LOST
